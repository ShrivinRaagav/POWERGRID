...
1. **Interval Coverage Assessment**:
   - The empirical Prediction Interval Coverage Probability (PICP) is **89.65%**, exceeding the nominal expected target of **80.0%**.
...
2. **Demand Spikes & Extreme Events Analysis**:
   - Out of `1,440` total test samples, **`149`** fell outside the P10-P90 bounds (`10.35%`).
   - Among the outliers, **`91` cases (61.1%)** represent under-predicted demand spikes where actual demand exceeded P90.
...
3. **Quantile Crossing & Monotonicity Verification**:
   - **`14`** quantile crossing violations were identified in the raw independent predictions.
...
- **Expected Outcome**: Will reduce missed demand spikes from `91` cases down to < 20 cases.
